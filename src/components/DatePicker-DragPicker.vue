    ...
    <div class="dialog-container">
      ...
      >
        <template v-for="(month, index) in calendars">
          <div
            class="slider_item" :style="getTransform(index)"
            @webkit-transition-end="onTransitionEnd(index)"
            @transitionend="onTransitionEnd(index)"
          >
            <div class="month">
              <span>{{month[1][0].date.substr(0, 7)}}</span>
            </div>
            <div class="days">
              <template v-for="week in month">
                <template v-for="day in week">
                  <div
                    class="day"
                    :class="{'not_current_month': !day.isCurrentMonth}"
                    @click.stop="dateClickHandle(day.date)"
                  >
                    <div class="date">{{day.date.split('-')[2]}}</div>
                    <div
                      class="event_num"
                      v-if="day.isCurrentMonth && day.eventNum"
                    >{{day.eventNum}}</div>
                  </div>
                </template>
              </template>
